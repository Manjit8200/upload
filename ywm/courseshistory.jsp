...
<title> Past Courses | Yoga, Meditation, Pranayama, Sound Healing,
...
<script	src="<%=request.getContextPath()%>/resources/admin/js/controller/historycourse.js"></script>
<style>
.contentDe {
  height: 100px;
  overflow: hidden;
...
<body ng-app="ywm" ng-controller="HistoryCourseCtrl">
	...
	<section style="padding: 90px 0 90px;" class="causes-section">
		<div class="container">
			...
			<div class="row">
				...
				<!-- Cause Block One -->
				<div ng-repeat="item in getAllHistoryCourseFront" class="cause-block-one col-md-4">
					<div class="inner-box">
						<div class="image">
							<a href="<%=request.getContextPath() %>/history-course-details?courseId={{item.courceId}}"><img src="{{item.courceImage}}" alt=""></a>
						</div>
						<div class="lower-content">
							<h4>
								<a href="<%=request.getContextPath() %>/history-course-details?courseId={{item.courceId}}"> {{item.courceTitle}} </a>
							</h4>
							...
							<div style="text-align: center; border: 1px dotted #aaa; padding: 6px 0px 6px 0px; border-radius: 50px; font-weight: 600; color: #212529;"
								class="text">
								Fee: Rs. {{item.courseFee}} <small>({{item.courceFeeValidity}})</small>
							</div>
							...
							<div style=" text-align: center; border: 1px dotted #aaa; padding: 6px 0px 6px 0px; border-radius: 50px; font-weight: 600; color: #212529;" class="text">
								<a href="<%=request.getContextPath() %>/history-course-details?courseId={{item.courceId}}">Know More</a>
							</div>
